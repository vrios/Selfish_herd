...
;;A simple implementation of Hamilton's selfish herd
;;(W.D. Hamilton, 1971, Geometry for the selfish herd, Journal of Theoretical Biology, 31(2), 295-311)
;;code derived from the Voronoi model in the NetLogo Model Library, adapted to make the points
;;move according to selfish herd principles,
;;Voronoi Model by Wilensky, U. (2006). NetLogo Voronoi model.
...
to be-selfish ;; points move towards closest neighbor only. randomization of order
  let current-neighbor min-one-of other points [distance myself]
  face current-neighbor
  ;; check if the point needs to move
  let x [xcor] of current-neighbor
  let y [ycor] of current-neighbor

  fd 10;; (distance current-neighbor) / 2
  ask spawners [ die ]
  ask updaters [ die ]
  ask patch-here [ make-spawners ]
  while [any? spawners or any? updaters]
  [
  ask spawners [ spawn ]
  ask updaters [ update ]
  ]
...
;Voronoi Model Copyright 2006 Uri Wilensky.
;;Selfish Herd Model Copyright Vitor Rios, Creative Commons Attribution-NonCommercial-ShareAlike 3.0
;; License. To view a copy of this license, visit https://creativecommons.org/licenses/by-nc-sa/3.0/
@#$#@#$#@
GRAPHICS-WINDOW
210
10
